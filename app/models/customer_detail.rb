class CustomerDetail < ApplicationRecord
    ...
    after_create :create_engineer_table
	...
	def create_engineer_table
       @engineer = EngineerDetail.create!(bill_no: self.bill_no, customer_name: self.customer_name, date: self.date, address: self.address,customer_no: self.customer_no, telephone_no: self.telephone_no,
        ...
        good_delivered: self.good_delivered, return_by_date: self.return_by_date ,defect_description: self.defect_description, accessory: self.accessory, repair_description: self.repair_description,customer_detail_id: self.id, employee_id: self.employee.id,
        condition_code: self.condition_code, repair_code: self.repair_code, compalint_type: self.compalint_type, imei_1: self.imei_1, imei_2: self.imei_2,
        status: self.status, estimated_cost: self.estimated_cost,balance_cost: self.balance_cost, advanced_paid: self.advanced_paid)
    end

    def self.search_customer_search(search)
     CustomerDetail.where('lower(customer_name) LIKE ?', "%#{search}%")
    end

    def self.search(search)
     CustomerDetail.where('lower(customer_name) LIKE ? OR (bill_no) LIKE ?', "%#{search}%", "%#{search}%")
    end
